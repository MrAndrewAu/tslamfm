"""
v7 candidate: v6-canonical-4y + two microstructure factors.

STATUS: REJECTED (April 2026)
  Walk-forward OOS R^2 dropped from v6's 0.662 to:
    - 0.492 with both factors (-17.0pp, n=190)
    - 0.620 with up_down_vol_4w only (-4.2pp, n=208)
    - 0.360 with off_exch_ratio_zscore only (-30.2pp, n=190)
  In-sample R^2 rose to 0.89 = textbook overfitting signature.
  Both factors had strong PROMISING verdicts on the full 4-year sample
  but their OOS-period sub-correlation flipped sign or shrank to noise,
  indicating the relationships are regime-dependent (2022-2024 only).
  See analyze_microstructure_signal.py for the candidate-test results.

Kept as documentation. To re-test, just run this script.

Added factors (both PROMISING in analyze_microstructure_signal.py):
  - up_down_vol_4w        rolling 4w sum(up-day vol)/sum(down-day vol)
  - off_exch_ratio_zscore 52w z-score of FINRA-TRF / consolidated volume

Writes to *_v7.json so the production v6 outputs are not overwritten.
"""
...
FINRA = OUT_DIR / "finra_volume.csv"
...
# --- v7 microstructure factors ---
print("Building microstructure factors...")
daily_tsla = yf.Ticker("TSLA").history(start=START, end=END, interval="1d")
_idx = pd.to_datetime(daily_tsla.index)
if getattr(_idx, "tz", None) is not None:
    _idx = _idx.tz_localize(None)
daily_tsla.index = _idx
daily_tsla = daily_tsla[["Close", "Volume"]].rename(columns=str.lower)
daily_tsla["ret"] = daily_tsla["close"].pct_change()
daily_tsla["up_vol"] = np.where(daily_tsla["ret"] > 0, daily_tsla["volume"], 0.0)
daily_tsla["dn_vol"] = np.where(daily_tsla["ret"] < 0, daily_tsla["volume"], 0.0)
up_w = daily_tsla["up_vol"].resample("W-FRI").sum()
dn_w = daily_tsla["dn_vol"].resample("W-FRI").sum()
up_down_vol_4w = (up_w.rolling(4, min_periods=2).sum()
                  / dn_w.rolling(4, min_periods=2).sum().replace(0, np.nan))

finra = pd.read_csv(FINRA, index_col="date", parse_dates=["date"]).sort_index()
merged = finra.join(daily_tsla[["volume"]], how="inner")
merged["off_exch_ratio"] = (merged["finra_total_vol"] / merged["volume"]).clip(upper=1.0)
off_exch_w = merged["off_exch_ratio"].resample("W-FRI").mean()
off_exch_z = (off_exch_w - off_exch_w.rolling(52, min_periods=20).mean()) \
             / off_exch_w.rolling(52, min_periods=20).std()

f["up_down_vol_4w"] = up_down_vol_4w.reindex(f.index)
f["off_exch_ratio_zscore"] = off_exch_z.reindex(f.index)
...
FORCED = ["log_QQQ", "log_DXY", "log_VIX", "NVDA_excess", "ARKK_excess",
          "up_down_vol_4w", "off_exch_ratio_zscore"]
...
with open(OUT_DIR / "model_v7.json", "w") as fh:
    json.dump(model_obj, fh, indent=2)
print(f"Wrote {OUT_DIR / 'model_v7.json'}")
...
with open(OUT_DIR / "history_v7.json", "w") as fh:
    json.dump(history, fh)
print(f"Wrote {OUT_DIR / 'history_v7.json'} ({len(history)} rows)")

# Console summary
print(f"\nv7-microstructure | factors={len(factors)} | n={len(f)}")
