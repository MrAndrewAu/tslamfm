"""
Walk-forward OOS test: v6 baseline vs v6 + AAPL_excess_vs_QQQ.

STATUS: REJECTED -- definitive saturation confirmation (April 2026)
  AAPL_excess had the strongest univariate lagged corr in the entire
  exploration: corr_t+1 = +0.544 (p<0.001, n=209). OOS sub-period was
  +0.192 (p=0.115) -- borderline but right sign.

  Walk-forward results:
    v6 baseline:              In R2=0.853  OOS R2=0.6615  MAE=7.71%
    v6 + AAPL_excess:         In R2=0.914  OOS R2=0.5835  MAE=9.56%  (-7.80pp)

  In-sample R^2 lift was +6.0pp (largest of any candidate ever tested).
  OOS R^2 dropped -7.8pp. Pure multicollinearity overfit: AAPL adds
  co-movement information that NVDA/ARKK already encode; the redundancy
  destabilizes rolling refits.

KEY FINDING: This is the falsification test that proves v6's tech-
co-movement factor set is SATURATED. Even the strongest possible
univariate tech signal cannot beat NVDA + ARKK in walk-forward. Future
factor candidates that share variance with NVDA/ARKK/QQQ are essentially
guaranteed to fail. Look for genuinely orthogonal economic stories
(macro, fundamentals, regime indicators) instead.
"""
...
f["AAPL_excess"] = residualize(np.log(df["AAPL"]), f["log_QQQ"])
...
r2_v61, _ = walk_forward(f, V61, "v6 + AAPL_excess (v6.1)")
delta = (r2_v61 - r2_v6) * 100
print(f"\n  Delta OOS R^2: {delta:+.2f}pp")
print(f"  Acceptance bar: >= +2.00pp")
verdict = "ACCEPT (promote to v6.1)" if delta >= 2.0 else "REJECT (stay on v6)"
