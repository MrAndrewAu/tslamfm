"""
Probe: Copper (HG=F) as additive v6.4 factor.

Economic story: Copper is both a direct EV battery input (wiring, motors)
and a leading global-PMI indicator. TSLA's cost structure and demand cycle
are both exposed to copper. Distinct from RBOB (consumer gas price),
DXY (dollar), and the yield curve (rate regime).

Transforms tested:
  copper_zscore_52w  — 52-week log z-score (primary)
  copper_yoy_z       — year-over-year log-return z-score
  copper_mom4_z      — 4-week log-return z-score (fast signal)

Acceptance gate: +1pp walk-forward OOS R² lift (May 2026 threshold).
Robustness: tenant-swap (drop RBOB, drop DXY) + OOS sub-period corr.
"""
...
def compute_vif(frame, new_factor):
    """VIF of new_factor against v6.4 forced factors."""
    cols = FORCED_V64 + [new_factor]
...
def lagged_corr_vs_resid(candidate_col, frame, resid_v64):
    c_lag = frame[candidate_col].shift(1)
...
def oos_subcorr(candidate_col, frame, resid_v64):
    """Correlation of candidate with v6.4 residual in the OOS window only."""
    oos_idx = frame.index[frame.index >= pd.Timestamp(OOS_START)]
    cand = frame.loc[oos_idx, candidate_col].dropna()
    resid = resid_v64.reindex(cand.index).dropna()
    common = cand.index.intersection(resid.index)
    if len(common) < 10:
        return np.nan, np.nan, 0
    c, p = stats.pearsonr(cand.loc[common], resid.loc[common])
...
print("=" * 68)
print("Copper (HG=F) probe vs v6.4")
print("=" * 68)
...
core_df = (pd.DataFrame({k: raw[k] for k in raw if k != "COPPER"})
...
# ═══════════════════════════════════════════════════════════════════════════════
# 3. Build copper transforms
# ═══════════════════════════════════════════════════════════════════════════════

print("\n[2] Building copper transforms...")

if "COPPER" in raw:
    cu = raw["COPPER"].reindex(f_eq.index, method="ffill")
    log_cu = np.log(cu)

    f_eq["copper_zscore_52w"] = (
        (log_cu - log_cu.rolling(52, min_periods=20).mean()) /
         log_cu.rolling(52, min_periods=20).std()
    )
    raw_yoy = log_cu.diff(52) * 100
    f_eq["copper_yoy_z"] = (
        (raw_yoy - raw_yoy.rolling(52, min_periods=20).mean()) /
         raw_yoy.rolling(52, min_periods=20).std()
    )
    raw_mom4 = log_cu.diff(4) * 100
    f_eq["copper_mom4_z"] = (
        (raw_mom4 - raw_mom4.rolling(52, min_periods=20).mean()) /
         raw_mom4.rolling(52, min_periods=20).std()
    )
    # Excess vs DXY (copper often moves inversely to dollar; residualize to isolate
    # the real-demand component orthogonal to currency effects)
    cu_aligned = log_cu.reindex(f_eq.index).dropna()
    dxy_aligned = base["log_DXY"].reindex(cu_aligned.index)
    cu_excess = residualize(cu_aligned, dxy_aligned)
    f_eq["copper_excess_vs_DXY"] = cu_excess.reindex(f_eq.index)

CANDIDATES = [
    "copper_zscore_52w",
    "copper_yoy_z",
    "copper_mom4_z",
    "copper_excess_vs_DXY",
]
...
print(f"  v6.4 baseline  OOS R²={base_r2:.4f}  MAE={base_mae:.2f}%  n={base_n}")

# v6.4 full-sample residual (for lagged-corr + OOS sub-period checks)
...
print(f"\n  {'factor':<30}  {'corr_lag1':>9}  {'p':>6}  {'VIF':>6}  screen")
print("  " + "-" * 64)
...
for col in CANDIDATES:
    sub = f_eq[[col]].dropna()
    if len(sub) < 30:
        print(f"  {col:<30}  insufficient data")
        continue
    corr, p = lagged_corr_vs_resid(col, f_eq, resid_v64)
    vif = compute_vif(f_eq.dropna(subset=[col]), col)
    ...
    print(f"  {col:<30}  {corr:>+9.4f}  {p:>6.4f}  {vif:>6.2f}  {flag}")
    screen_results[col] = dict(corr=corr, p=p, vif=vif,
                                corr_pass=corr_pass, vif_pass=vif_pass)

print("\n[5] Walk-forward for all transforms (proceeding regardless of screen):")
print(f"\n  {'factor':<30}  {'OOS R²':>7}  {'MAE':>7}  {'delta':>8}  verdict")
print("  " + "-" * 72)
...
for col in CANDIDATES:
    ...
    print(f"  {col:<30}  {r2:>7.4f}  {mae:>6.2f}%  {delta:>+8.2f}pp  "
          f"{verdict}  ({vif_str})")
    wf_results[col] = dict(r2=r2, mae=mae, delta=delta, verdict=verdict)

# ═══════════════════════════════════════════════════════════════════════════════
# 6. OOS sub-period correlation for primary transform
# ═══════════════════════════════════════════════════════════════════════════════

primary = "copper_zscore_52w"
if primary in f_eq.columns:
    ...
    # OOS contemporaneous (no lag — testing in-period alignment)
    c_oos, p_oos, n_oos = oos_subcorr(primary, f_eq, resid_v64)
    print(f"  OOS only contemporaneous corr: {c_oos:+.4f}  p={p_oos:.4f}  n={n_oos}")

# ═══════════════════════════════════════════════════════════════════════════════
# 7. Robustness tenant-swaps (only if primary passes +1pp gate)
# ═══════════════════════════════════════════════════════════════════════════════

primary_res = wf_results.get(primary, {})
if primary_res.get("delta", -999) >= BARRIER_PP:
    print(f"\n[7] Robustness tenant-swap tests for {primary}:")
    print(f"\n  {'test':<48}  {'OOS R²':>7}  {'delta':>8}")
    print("  " + "-" * 66)

    swap_tests = [
        ("v6.4 + copper_zscore_52w (primary)",
         FORCED_V64 + [primary]),
        ("v6.4 - RBOB + copper_zscore_52w",
         [f for f in FORCED_V64 if f != "RBOB_zscore_52w"] + [primary]),
        ("v6.4 - DXY + copper_zscore_52w",
         [f for f in FORCED_V64 if f != "log_DXY"] + [primary]),
        ("v6.4 - VIX + copper_zscore_52w",
         [f for f in FORCED_V64 if f != "log_VIX"] + [primary]),
        ("v6.4 - curve + copper_zscore_52w",
