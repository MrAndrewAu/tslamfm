"""
Test individual equities (PLTR, COST, ...) as candidate v7 factors.

Mirrors analyze_crypto_signal.py — same discipline:
  1. Pull weekly close.
  ...
  5. PROMISING needs to clear OOS sub-period sanity check before walk-forward.

Usage:
    python scripts/analyze_equity_signal.py PLTR COST
    python scripts/analyze_equity_signal.py PLTR COST F GM AAPL
"""
...
def main():
    ...
    for sym in syms:
        ...
        for name, s in factors.items():
            ...
            r["verdict"] = verdict(r)
            rows.append(r)
        print(f"{'factor':<32}{'n':>5}{'corr_t':>10}{'p':>8}{'corr_t+1':>11}{'p':>8}{'corr_oos':>11}{'p':>8}  verdict")
        print("-" * 122)
        def f(x, fmt="{:>+.3f}"):
