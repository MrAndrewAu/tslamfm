"""
v7 candidate: v6-canonical-4y + crypto factor (BTC or ETH excess vs QQQ).

STATUS: REJECTED (April 2026)
  Walk-forward OOS R^2 dropped from v6's 0.662 to:
    - 0.349 with BTC_excess  (-31.3pp, n=209)
    - 0.574 with ETH_excess  (-8.8pp,  n=209)
  Same overfitting pattern as microstructure factors:
  full-sample lagged correlation PROMISING (BTC: -0.20 p=0.005;
  ETH: -0.22 p=0.002) but OOS-period sub-correlation insignificant
  or sign-flipped. Regime-dependent — captured 2022-2024 dynamics that
  did not carry into 2025-2026.
  See analyze_crypto_signal.py for the candidate-test results.

Kept as documentation. To re-test, just run this script (currently
wired with BTC_excess; flip FORCED to ETH_excess to re-test that variant).
"""
...
S = {k: wk(v) for k, v in {
    ...
    "NVDA": "NVDA", "ARKK": "ARKK", "BTC": "BTC-USD", "ETH": "ETH-USD"
}.items()}
...
btc_a,  btc_b,  btc_resid  = residualize(np.log(df["BTC"]),  f["log_QQQ"])
eth_a,  eth_b,  eth_resid  = residualize(np.log(df["ETH"]),  f["log_QQQ"])
f["NVDA_excess"] = nvda_resid
f["ARKK_excess"] = arkk_resid
f["BTC_excess"]  = btc_resid
f["ETH_excess"]  = eth_resid
...
FORCED = ["log_QQQ", "log_DXY", "log_VIX", "NVDA_excess", "ARKK_excess", "BTC_excess"]
...
with open(OUT_DIR / "model_v7crypto.json", "w") as fh:
    json.dump(model_obj, fh, indent=2)
print(f"Wrote {OUT_DIR / 'model_v7crypto.json'}")
...
with open(OUT_DIR / "history_v7crypto.json", "w") as fh:
    json.dump(history, fh)
print(f"Wrote {OUT_DIR / 'history_v7crypto.json'} ({len(history)} rows)")

# Console summary
print(f"\nv7-crypto | factors={len(factors)} | n={len(f)}")
